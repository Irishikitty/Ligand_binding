...
def get_indices_of_k_smallest(arr, k = 255):
...
if __name__=="__main__":
    ...
    for key, value in dataset.items():
        ...
        for data in tqdm(value):
            ...
            # Do the following if the ligand has ONLY one atom
            if len(axes) == 1:
                print(pdbid, chainID, pos, ligand)
                file_name = 'pdb_files_all_new/' + pdbid.lower() + '.pdb'
                result = check_models(file_name)
                # new add:  util   return atom_label[resname][atom] KeyError: '  G'
                try:
                    if result:
                    ...
                    if pdb_atoms4chainIDs:
                        ...
                        for chain, atoms_data in pdb_atoms4chainIDs.items():
                            ...
                            min_dist_chain[chain] = np.min(pairwise_distances(axes, np.array(atomCoords).T[1:].T))
                        ...
                        try:
                            assert len(atomCoords_final) > 255
                            distance = pairwise_distances(axes, np.array(atomCoords_final).T[1:].T)
                            index_protein_atoms = list(set(get_indices_of_k_smallest(distance, 255)))
